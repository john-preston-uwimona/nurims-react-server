...
def get_manufacturer_records(conn, log, request):
    cur = None
    try:
        cur = conn.cursor()
        ...
        if "item_id" in request:
            log.trace("searching for manufacturer records in items table with item_id={}".format(request["item_id"]))
            cur.execute("""SELECT * from item, item_topic_registry where
                        ...
                        (Constants.MANUFACTURER_TOPIC, Constants.MANUFACTURER_RECORD_TYPE, request["item_id"],))
            row = cur.fetchone()
            if row is not None:
                data.append({
                    "item_id": row['item_id'],
            ...
            log.trace("searching for manufacturer records in items table")
            cur.execute("""SELECT * from item, item_topic_registry where
                           item.item_topic_id = item_topic_registry.item_topic_id and item_topic=? and type=?""",
                        (Constants.MANUFACTURER_TOPIC, Constants.MANUFACTURER_RECORD_TYPE))
            row = cur.fetchone()
...
def save_manufacturer_record(conn, log, request):
    cur = None
    ...
    try:
        ...
        if request["item_id"] == -1:
            cur.execute("""SELECT item_topic_id from item_topic_registry where item_topic=?""",
                        (Constants.MANUFACTURER_TOPIC,))
            row = cur.fetchone()
            if row is None:
                request.update({
                    "response": {
                        "message": "Cannot find manufacturer topic in item_topic_registry",
                        "status": 1
                    }
                })
                return
            log.trace("inserting a new manufacturer record")
            item_topic_id = row['item_topic_id']
            ts = datetime.now().isoformat()
            cur.execute("""INSERT into item (item_topic_id,title,type,withdrawn,last_modified) VALUES (?,?,?,?,?)""",
                        (item_topic_id, request["nurims.title"], Constants.MANUFACTURER_RECORD_TYPE, request["nurims.withdrawn"], ts,))
            ...
            if row is None:
                request.update({
                    "response": {
                        "message": "Cannot find newly created manufacturer record",
                        "status": 1
                    }
                })
                return
            ...
            log.trace("searching for manufacturer in items table with item_id = {}".format(request["item_id"]))
            cur.execute("""SELECT COUNT(*) as n from item, item_topic_registry where
                           ...
                           item_id=? and type=?""",
                        (Constants.MANUFACTURER_TOPIC, request["item_id"], Constants.MANUFACTURER_RECORD_TYPE))
            row = cur.fetchone()
            ...
            if rowcount == 0:
                request.update({
                    "response": {
                        "message": "No manufacturer records with item_id = {}".format(request["item_id"]),
                        "status": 1
                    }
                })
                return
                ...
                request.update({
                    "response": {
                        "message": "Multiple manufacturer records with item_id = {}".format(request["item_id"]),
                        "status": 1
                    }
                })
                return
        ...
        request.update({
            "response": {
                Constants.MANUFACTURER_TOPIC: data,
                "message": "",
                "status": 0
            }
        })
